...
CURRENT=$(docker exec postgres-db psql -U nik -d mydb -t -A -c \
...
echo "  docker exec postgres-db psql -U nik -d mydb -c \"SELECT id, production_line_id, state, ts FROM machine_states WHERE production_line_id = ${LINE_ID} ORDER BY ts DESC LIMIT 3;\""
